...
[NOTE]
====
It's essential to carefully evaluate your project's requirements and constraints before deciding to implement a separated tiers architecture, because it comes with trade-offs:
...
All application data is stored in the backend database.
...
* Access the Backend application by navigating to http://localhost:8081[^] in your web browser and log in as `admin` user with `admin` password.
...
To run on the non-default port 8081 and prevent cookie conflicts with the Frontend application, the Backend application requires the following properties:

[source,properties,indent=0]
.backend-app/src/main/resources/application.properties
----
include::example$/backend-app/src/main/resources/application.properties[tags=port]
----
...
To support serialization of `User` entity to JSON for storing OAuth2 tokens in the database, the backend application defines a mixin class and uses it in the `JdbcOAuth2AuthorizationServiceJsonMapperCustomizer` bean:

[source,java,indent=0]
.backend-app/src/main/java/com/company/backend/security/OAuth2TokenUserMixin.java
----
include::example$/backend-app/src/main/java/com/company/backend/security/OAuth2TokenUserMixin.java[]
----

[source,java,indent=0]
.backend-app/src/main/java/com/company/backend/BackendAppApplication.java
----
include::example$/backend-app/src/main/java/com/company/backend/BackendAppApplication.java[tags=tokenObjectMapperCustomizer]
----

[[frontend-application]]
==== Frontend Application

The Frontend application includes the xref:rest-ds:index.adoc[] add-on for authenticating users through the Backend application and accessing its data. The following dependencies are added to `build.gradle`:

[source,groovy,indent=0]
.frontend-app/build.gradle
----
include::example$/frontend-app/build.gradle[tags=dependencies]
----
<1> REST DataStore implementation.
<2> Implementation of the user settings and filter configuration persistence based on the REST DataStore.

The following dependencies have been removed in comparison to a standard application that interacts directly with the database:
...
The REST DataStore is configured in the `application.properties` file of the Frontend application, along with its connection parameters:
...
The `jmix.restds.ui-config-store` property defines the REST data store that will be used for user settings and filter configuration persistence.

To access files located in the Backend application's file storage, the Frontend application defines the xref:rest-ds:index.adoc#file-storage[RestFileStorage] bean:

[source,java,indent=0]
.frontend-app/src/main/java/com/company/frontend/FrontendAppApplication.java
----
include::example$/frontend-app/src/main/java/com/company/frontend/FrontendAppApplication.java[tags=backendFileStorage]
----
...
The use of fetch plans in the Frontend application is the same as when working directly with databases. These xref:rest:entities-api/load-entities.adoc#inline-fetch-plans[inline fetch plans] are serialized in JSON and passed to the Backend REST endpoints.

[[files]]
=== Files

The `User` entity in both applications has the `picture` property which references a file in a file storage:

[source,java,indent=0]
.backend-app/src/main/java/com/company/backend/entity/User.java
----
include::example$/backend-app/src/main/java/com/company/backend/entity/User.java[tags=picture]
----

[source,java,indent=0]
.frontend-app/src/main/java/com/company/frontend/entity/User.java
----
include::example$/frontend-app/src/main/java/com/company/frontend/entity/User.java[tags=picture]
----

In the Backend application, this attribute contains a reference to the xref:files:file-storage.adoc#local-fs[local file storage].

In the Frontend application, the value of this attribute is automatically transformed to a reference to the xref:rest-ds:index.adoc#file-storage[REST file storage], which transparently transmits files to and from the Backend application. The REST file storage is configured using the following bean:
...
To use the REST Files API, the `rest.fileDownload.enabled` and `rest.fileUpload.enabled` specific policies must be enabled for the user. It is done in the `HrManagerRole` class of the Backend application:

[source,java,indent=0]
.backend-app/src/main/java/com/company/backend/security/HrManagerRole.java
----
include::example$/backend-app/src/main/java/com/company/backend/security/HrManagerRole.java[tags=role;files]
----

[[views]]
=== Views

The Frontend views are mostly identical to the views of the basic Onboarding application, with the following difference.

`MyOnboardingView` defines the query in the JSON format of the REST API instead of JPQL:

[source,xml,indent=0]
.frontend-app/src/main/resources/com/company/frontend/view/myonboarding/my-onboarding-view.xml
----
...
[source,java,indent=0]
.frontend-app/src/main/java/com/company/frontend/security/RestLoginViewSupport.java
----
